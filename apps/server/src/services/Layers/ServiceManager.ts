...
const makeServiceManager = Effect.fn("makeServiceManager")(function* () {
  ...
  const startDockerLogFollower = (serviceId: string): ChildProcess | undefined => {
    ...
    const child = spawn(
      ...
      { stdio: ["ignore", "pipe", "pipe"], env: { ...process.env, ...freshEnv() } },
    );
  ...
  const freshEnv = (): Record<string, string> => {
    ...
    return parseDotenv(config.envFile);
  };
      ...
      if (newStatus !== runtime.status) {
        ...
        if (newStatus === "stopped") {
          ...
          if (!runtime.logFollower) {
            runtime.logFollower = startDockerLogFollower(id);
          }
        }
        changed = true;
      }
  ...
  const startServiceInternal = async (serviceId: string): Promise<void> => {
    ...
    if (def.type === "docker") {
      const result = await dockerCompose(["up", "-d", "--build", serviceId]);
      ...
      const child = spawn(def.command ?? "echo 'no command'", {
        ...
        shell: true,
        detached: true,
        env: { ...process.env, PORT: undefined, ...freshEnv() },
  ...
  const startTaskInternal = (taskId: string): void => {
    ...
    const runTick = () => {
      ...
      exec(def.command, { env: { ...process.env, ...freshEnv() }, timeout: 30_000 }, (err) => {
